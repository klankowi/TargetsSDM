...
# Start at the habitat covariates part and skip the base model?
hab_cov_start<- TRUE
```
...
As we mentioned above, we don't expect that all users are going to use these same prep functions. That said, it is important that the structure of the `all_tows` dataframe and the `all_occu` dataframe have a similar structure to use the other functions. **Improvements could certainly be made here to make things even more flexible**. At a minimum, `all_tows` should have columns named: `EST_YEAR`,  `DECDEG_BEGLAT`, `DECDEG_BEGLON`, `ID`, `DATE` (in YYY-MM-DD format) and `SEASON` and `SURVEY` (if applicable and fitting seasonal model or a model using survey as a catchability factor). `all_occu` should have columns named `ID`, `PRESENCE`, `BIOMASS`, `ABUNDANCE`, `SURVEY` and then a column with a species ID code. 
```{r, eval = eval_use}
...
For this vignette, we will go ahead and enhance the tow data by extracting depth, a seasonal sea surface temperature and a seasonal bottom temperature at each of the tow locations. Additionally, we will use some conveinance wrapper functions (`static_extract_wrapper` and `dynamic_2d_extract_wrapper`) that we wrote. These wrappers help process multiple different static raster layers or dynamic raster stacks that are stored in list. We will run the `static_extract_wrapper` (and `static_extract`) function first and then run the `dynamic_2d_extract_wrapper` and `dynamic_2d_extract` function.
...
vast_settings<- vast_make_settings(extrap_grid = vast_extrap_grid_nona, n_knots = 200, FieldConfig = field_config, RhoConfig = rho_config, OverdispersionConfig = c(0, 0), bias.correct = FALSE, knot_method = "grid", inla_method = "Barrier", Options = c("Calculate_Range" = TRUE), strata.limits = strata_use)
```
...
```{r, eval = eval_use}
vast_fitted<- vast_fit_sdm(vast_build_adjust = vast0, nice_category_names = "Atlantic_halibut", index_shapes = index_area_shapes, spatial_info_dir = here::here(""), out_dir = here::here("", "results/mod_fits"))
```
...
obs_pred0<- vast_get_point_preds(vast_fit = vast_fitted, use_PredTF_only = FALSE, nice_category_names = "Atlantic_halibut", out_dir = here::here("", "results/tables"))
...
taylor_diag0<- taylor_diagram_func(dat = obs_pred0, obs = "Biomass", mod = "Predicted_Biomass", pt.col = "#d95f02", out.file = paste0(here::here("", "results/plots_maps"), "/Atlantic_halibut_base_TaylorDiagram.jpg"))
taylor_diag0
```
```{r, eval = !eval_use, echo = FALSE, fig.cap = "VAST base model Taylor Diagram to assess model predictive skill."}
knitr::include_graphics("results/plots_maps/Atlantic_halibut_TaylorDiagram.jpg")
```
...
biomass_indices<- get_vast_index_timeseries(vast_fit = vast_fitted, all_times = unique(vast_sample_data$Year), nice_category_names = "Atlantic_halibut", index_scale = c("raw"), out_dir = here::here("", "results/tables"))

plot_vast_index_timeseries(index_res_df = biomass_indices, index_scale = "raw", nice_category_names = "Atlantic_halibut", nice_xlab = "Year", nice_ylab= "Biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = here::here("", "results/plots_maps"))
```

```{r, eval = !eval_use, echo = FALSE, fig.cap = "'Stratified' biomass index"}
knitr::include_graphics("results/plots_maps/Biomass_Index_raw_Atlantic_halibut.jpg")
```
...
Another thing we might be interested in summarizing the distribution and looking at changes in the center of gravity, as well as center of longitude and latitude (with uncertainty). 
```{r, eval = eval_use}
vast_cog_plot<- vast_plot_cog(vast_fit = vast_fitted, all_times = unique(vast_sample_data$Year), summarize = TRUE, land_sf = land_use, xlim = xlim_use, ylim = ylim_use, nice_category_names = "Atlantic_halibut", land_color = "#d9d9d9", color_pal = NULL, out_dir = here::here("", "results/plots_maps"))
vast_cog_plot
```
```{r, eval = !eval_use, echo = FALSE, fig.cap = "Predicted density"}
knitr::include_graphics("results/plots_maps/COG_Atlantic halibut")
```

## Stage 8: Use the model to make predictions
Armed with a fitted model, we might be interested in using the model to make predictions. This becomes more interesting as we add in habitat and catchability covariates and then have new covariate information to use -- for example projected sea surface temperatures from a global climate model. So, we save the prediction discussion until then. 
...
Let's start by adding in some habitat covariates and an autoregressive structure to the model intercepts to facilitate using the model to make predictions or forecasts. **As far as I know, without adjusting this structure we will not be able to make any predictions beyond the time horizon used in fitting the model.** Hopefully, this process looks somewhat familiar to those who have fit an "environment-only" species distribution model where we are including some static/dynamic values for variables that we think influence a species occurrence pattern. With many modeling approaches, all of this information can be in one dataframe -- similar to our tidy model dataframe. With VAST, we actually want to separate out a specific "covariate data" dataframe. 

```{r, eval = eval_use}
# Read in data we need if we are picking the vignette up from this point and haven't run the previous section
if(hab_cov_start){
  # Read it in
...
tidy_mod_data_run<- tidy_mod_data %>%
  ...
  # Read in other supporting files we will need
  region_shape<- st_read(here::here("", "data/supporting/region_shapefile/full_survey_region.shp"))
...
We will also need to modify this "covariate" dataframe and our "sample" dataframe slightly. This modification involves providing a dummy observation for any time step we might be interested in predicting (or forecasting). Importantly, we change the `Pred_TF` value for this observation from 0 to 1 to signal TMB to only use the observation in the predicted component and not in estimating the likelihood. This becomes more important later on as we bring in climate model projection data. For now, though, we are just going to imagine we are interested in applying the model to forecast to 2020, so we need at least one observation for 2020 in the sample and covariate dataframes. **Importantly, the number of observations supplied will influence derived products, like stratified biomass indices**
```{r, eval = eval_use}
vast_sample_data<- data.frame("Year" = tidy_mod_data_run$EST_YEAR, "Lat" = tidy_mod_data_run$DECDEG_BEGLAT, "Lon" = tidy_mod_data_run$DECDEG_BEGLON, "Biomass" = tidy_mod_data_run$BIOMASS, "Swept" = ifelse(tidy_mod_data_run$SURVEY == "NMFS", 0.0384, 0.0404), "Pred_TF" = rep(0, nrow(tidy_mod_data_run)))

samps<- sample(1:nrow(vast_covariate_data), size = 1, replace = FALSE)
dummy_dat_cov<- vast_covariate_data[samps,]
dummy_dat_cov$Year<- 2020
vast_covariate_data_aug<- rbind(vast_covariate_data, dummy_dat_cov)

dummy_dat<- vast_sample_data[samps,]
dummy_dat$Year<- 2020
dummy_dat$Pred_TF<- 1
...
The last step is to modify our model settings slightly by adding an autoregressive structure to the model intercepts. This is done by changing the `rho_config` structure and is necessary if we want to make predictions beyond the time horizon of our data.
...
# Now, call the settings function -- knot_method = "samples" throws errors! Also, bias.correct = FALSE for speed, but need this on in practice given somewhat uneven sampling design with two surveys.
vast_settings_fore<- vast_make_settings(extrap_grid = vast_extrap_grid_nona, n_knots = 200, FieldConfig = field_config, RhoConfig = rho_config, OverdispersionConfig = c(0, 0), bias.correct = FALSE, knot_method = "grid", inla_method = "Barrier", Options = c("Calculate_Range" = TRUE), strata.limits = strata_use)
```
...
-  An intercept term for each time step (e.g, year), estimated as random effects following a random walk (40 now instead of 39 with the base model)
...
```{r, eval = eval_use}
# Build
vast0_hab_covs<- vast_build_sdm(settings = vast_settings_fore, extrap_grid = vast_extrap_grid_nona, sample_data = vast_sample_data_aug, covariate_data = vast_covariate_data_aug, X1_formula = hab_formula, X2_formula = hab_formula, Q1_formula = NULL, Q2_formula = NULL, Xconfig_list = vast_coveff_habcovs, X_contrasts = NULL, index_shapes = index_area_shapes, spatial_info_dir = here::here(""))
```

After confirming everything looks right, we can fit the new model. 
```{r, eval = eval_use}
# Fit
vast_fitted_hab_covs<- vast_fit_sdm(vast_build_adjust = vast0_hab_covs, nice_category_names = "Atlantic_halibut_habcovs", index_shapes = index_area_shapes, spatial_info_dir = here::here(""), out_dir = here::here("", "results/mod_fits"))
```
...
if(hab_cov_start){
  ...
  obs_pred0<- readRDS(here::here("", "results/tables/Atlantic_halibut_obs_pred.rds"))
  auc0<- AUC(y_pred = obs_pred0$Predicted_ProbPresence, y_true = obs_pred0$Presence)
}

# Get observation and prediction dataset
obs_pred_habcovs<- vast_get_point_preds(vast_fit = vast_fitted_hab_covs, use_PredTF_only = FALSE, nice_category_names = "Atlantic_halibut_habcovs", out_dir = here::here("", "results/tables"))
...
Another thing that might of interest now is visualizing the fitted smooth functions defining the relationship between species occurrence and the environmental variables. To do that, we can run our `get_vast_covariate_effects` and `plot_vast_covariate_effects` functions. 
```{r, eval = eval_use}

vast_habcovs_effs<- get_vast_covariate_effects(vast_fit = vast_fitted_hab_covs, params_plot = c("Depth", "SST_seasonal", "BT_seasonal"), params_plot_levels = 100, effects_pad_values = c(), nice_category_names = "Atlantic_halibut", out_dir = here::here("", "results/tables"))

# Warnings are...interesting....
vast_habcovs_plot<- plot_vast_covariate_effects(vast_covariate_effects = vast_habcovs_effs, vast_fit = vast_fitted_hab_covs, nice_category_names = "Atlantic_halibut", out_dir = here::here("", "results/plots_maps"))
vast_habcovs_plot
```
...
As with our basic model, we can do a whole bunch of things after we've fit and evaluated/validated the model. For example, we could look at the estimated biomass and see how that compares to the basic model.
```{r, eval = eval_use}
if(hab_cov_start){
  # Get base predictions
  biomass_indices<- read_csv(here::here("", "results/tables/Biomass_Index_raw_Atlantic_halibut.csv")) %>%
    dplyr::select(., -X1)
}

biomass_indices_habcovs<- get_vast_index_timeseries(vast_fit = vast_fitted_hab_covs, all_times = unique(vast_sample_data_aug$Year), nice_category_names = "Atlantic_halibut_habcovs", index_scale = c("raw"), out_dir = here::here("", "results/tables"))
...
plot_vast_index_timeseries(index_res_df = biomass_indices_both, index_scale = "raw", nice_category_names = "Atlantic_halibut_habcovs_vs_base", nice_xlab = "Year", nice_ylab= "Biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = here::here("", "results/plots_maps"))

# Kind of ugly to look at, but could easily enough take and plot a difference or do whatever else you want.
```

Finally, we might be interested in using our model to make predictions. In this case, one of the easiest things to think about would be making a forecast using expected future sea surface and bottom temperature values. These values get passed to a `predict` style function as a `new_covariate_data` dataframe. Although we haven't necessarily gone through and actually gathered these "future" values from a regional ocean model or a global climate model, we can easily create something with a similar structure just using the data we already have and then pretend like the observations represent those from a physical ocean forecast model.

**An important note here about the prediction piece.** Just like other modeling frameworks, without considerable futzing around, predictions are only going to be possible for data with factor levels that the model has knowledge about. As a simple example, if we built a simple linear model and had "Age" as a factor and our data included "Ages" from 15 to 20, we wouldn't be able to make a prediction for a "21" year old -- that factor level is completely foreign to the model. This is why we included "2020" as a dummy variable, so that when we go to make predictions, we can predict to "2020" as the model knew about it. Looking ahead, for the climate scale projections, we would need a dummy observation for each year (and season) from 2020 to 2100.
...
Now that we have a "prediction" covariate dataframe that we know matches the original covariate dataframe exactly, we can try to use the beta/testing `predict` function.
```{r}
vast_forecast_density<- predict_vast(vast_fitted_sdm = vast_fitted_hab_covs, nice_category_names = "Atlantic_halibut_habcovs", predict_variable = "D_i", predict_category = 0, predict_vessel = 0, predict_covariates_df_all = pred_df, cov_names = c("Depth", "SST_seasonal", "BT_seasonal"), time_col = "Year", out_dir = here::here("", "results/prediction_df"))
str(vast_forecast_density)
```

**An aside about forecasting/projecting over longer time scales, especially when using spatio-temporal variability** This `predict_vast` function is certainly helpful. It does have some limitations though, particularly as we think about making predictions at longer time scales. If we followed what we did before, one potential way we could get these projections is to include one dummy observation for each season-year we are interested in and then we use the `predict_vast` function as above. In theory, that sounds great. In practice, this runs into some issues. With the current model structure, the model would try to estimate a spatio-temporal surface for every single time step of interest. Along with taking a **ton** of compute power, there's also a question of what that surface even means when we get out beyond a few times steps from the last observations let alone 80 years in the future. Additionally, only providing one observation per time step might result in some funky projected biomass indices -- these wouldn't be able to pull in any of the changing environmental data and would be purely driven by spatial/spatio-temporal variability and the intercepts. We could consider providing the gridded environmental data, however, this just makes the computation bit even more taxing and tends to cause memory issues and R crashes. Moreover, we can't post model-fit use the `predict` function to get predicted stratified biomass indices with uncertainties, which are calculated internally by `TMB`.

To summarize, we like the ability to turn on spatio-temporal variability (and possibly an auto-regressive structure to this random effect). Unfortunately, we are unable to do this at longer time scales without running into computational issues. So, how do we move forward to fit a model that allows us the capacity to make predictions using new gridded environmental data and have some potential to get stratified biomass indices, with uncertainty, for spatial regions of interest, while avoiding the computation issues trigged by including a dummy observation for every time step of interest?

A few options come to mind. One option is maybe we can "map off" the epsilon random effects for the longer time scales so that the model doesn't try to estimate these surfaces and their associated random effects. Looking into that a bit quickly in the code below.

```{r}
# Might need to just read in the fitted model...
vast_fitted_hab_covs<- readRDS(here::here("results/mod_fits/HabCovs_SingleAug_fitted_vast.rds"))

# Look at the Map argument...
str(vast_fitted_hab_covs$tmb_list$Map)

# Not entirely sure how we could do this here...but leaving it in in case this does seem like a promising route
```

A second option might be to use the fitted model and something like the "simulate" infrastructure to generate samples, and then make predictions from the samples, in turn generating "mean" predictions (local density, stratified biomass) and some measure of uncertainty? Let's start by taking a look at the simulate function and what it can do.
```{r}
# Let's look at the `simulate_data` function to see what it can do...
?simulate_data
```

The simulate data function is relatively straight forward argument wise with just three arguments: the output from `fit_model` the type of simulation to use, and then a random_seed generator. The type of simulation to use brings up some interesting questions. `type = 1` simulates measurement error/conditional simulator, where the new data are conditional on the estimated fixed and random effects. `type = 2` is an 'unconditional" simulator, where new random effects are simulated conditional on the estimated fixed effects and then new data are simulated. `type = 3` simulates new random and fixed effects from the joint precision matrix and then simulates new data. `type = 4` simulates new random effects conditional on fixed effects and original data from the internal Hessian matrix and then simulates new data. *I wonder if we could use different types to isolate different sources of uncertainty -- random vs. fixed effects model components, as an example?*

For our work, I *think* `type = 3` is going to make the most sense. Leveraging this type will require that we have set the `getJointPrecision` argument to TRUE, which is the default with our model fitting wrapper function but NOT the default with `vast::fit_model`. 
```{r}
# Check the getJointPrecision slot
vast_fitted_hab_covs$input_args$extra_args$getJointPrecision
```

if we dig a bit into the `simulate_data` function, we can see that there is a function inside this function that is then used to generate samples for `type = 3` using the following code chunk
```{r}
# First, the function -- a multivariate normal based on joint precision matrix?
rmvnorm_prec <- function(mu, prec, n.sims, random_seed) {
  if(FALSE){
    random_seed = 13
    mu = Obj$env$last.par.best # MLE estimates for all fixed and random effects
    prec = vast_fitted_hab_covs$parameter_estimates$SD$jointPrecision # Joint precision matrix
    n.sims = 1
  }
...
# Next, internally where it is used and substiting in our "vast_fitted_hab_covs" object for "fit" in the actual function
Obj<- vast_fitted_hab_covs$tmb_list$Obj
newpar<- rmvnorm_prec(mu = Obj$env$last.par.best, prec = vast_fitted_hab_covs$parameter_estimates$SD$jointPrecision, n.sims = 1, random_seed = random_seed)[, 1]

# Quick gut check -- do we have differences in newpar from the "last.par.best"?
head(newpar)
...
# Do we get differences in density -- "D_i". Yes.
dens_diffs<- summary(Return$D_i - vast_fitted_hab_covs$Report$D_i)
```

Okay, we've confirmed what this simulation process is doing. Now, thinking about what we actually want and then if these two things are going to align. Let's just take the situation where we are interested in forecasting the next time step, but we DON'T want to include this time as a dummy observation as it will mean the model tries to estimate the random effects and we run into our computing power issue. When we finish fitting the model, we have our MLE estimates. If we think about keeping the fixed effect parameters constant (e.g., use the MLE estimates for GAM smooths, catchability, etc), and keeping omega the same, the key piece is then "forecasting" the intercepts or epsilon, depending on the `rho_config` settings. We have these values for the last time step. For now, just looking into the intercepts, which have a random walk structure. As a reminder, a random walk is an AR1 process, with rho = 1.
```{r}
# First, just take a look at the relevant beta components. Our MLEs
vast_fitted_hab_covs$ParHat$beta1_ft

# Rho parameter
vast_fitted_hab_covs$ParHat$Beta_rho1_f

# Simulate "new" intercept based on random walk?
beta1_sim<- arima.sim(model = list(order=c(0, 1, 0)), n = 1, start.innov = vast_fitted_hab_covs$ParHat$beta1_ft)
```

That seems like it could work. What about if we get to something more complicated, like epsilon?
```{r}
# Again, seeing what we get for epsilon -- much more complicated than betas
str(vast_fitted_hab_covs$ParHat$Epsiloninput1_sff)

# Full array...dimensions knots, categories, time.
epsilon1_array<- vast_fitted_hab_covs$Report[[{{"Epsilon1_gct"}}]]

# Get the "last" time (minus one here as I didn't turn on autocorrelation for epsilon so the "last" surface is all Os)
epsilon1_last<- epsilon1_array[,,dim(epsilon1_array)[3]-1]
```

We've now got our "last-best" estimates for epsilon. How can we go about simulating the next year epsilon? It is not going to be as simple as a random walk for each of the locations as this is going to ignore the spatial autocorrelation among locations. 

```{r}
vast_fitted_hab_covs
```
