...
```{r, eval = eval_use}
vast_cog_plot<- vast_plot_cog(vast_fit = vast_fitted, land_sf = land_use, xlim = xlim_use, ylim = ylim_use, nice_category_names = "Atlantic halibut", land_color = "#d9d9d9", out_dir = here::here("", "results/plots_maps"))
vast_cog_plot
```
```{r, eval = !eval_use, echo = FALSE, fig.cap = "Predicted density"}
...
# Now, call the settings function -- knot_method = "samples" throws errors!
vast_settings_fore<- vast_make_settings(extrap_grid = vast_extrap_grid_nona, n_knots = 400, FieldConfig = field_config, RhoConfig = rho_config, OverdispersionConfig = c(0, 0), bias.correct = TRUE, knot_method = "grid", inla_method = "Barrier", Options = c("Calculate_Range" = TRUE), strata.limits = strata_use)
```
...
plot_vast_index_timeseries(index_res_df = biomass_indices_both, year_stop = 2019, index_scale = "raw", nice_category_names = "Atlantic halibut", nice_xlab = "Year", nice_ylab= "Biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = here::here("", "results/plots_maps"))

# Kind of ugly to look at, but could easily enough take and plot a difference or do whatever else you want.
```

Finally, we might be interested in using our model to make predictions. **Come back to this -- ** In this case, one of the easiest things to think about would be making a forecast using expected future sea surface and bottom temperature values. These values get passed to a `predict` style function as a `new_covariate_data` dataframe. Although we haven't necessarily gone through and actually gathered these "future" values from a regional ocean model or a global climate model, we can easily create something with a similar structure just using the data we already have and then pretending like the observations represent those from a physical ocean forecast model.
```{r, eval = eval_use}
...
```{r}
vast_forecast_density<- predict_vast(vast_fitted_sdm = vast_fitted_hab_covs, nmfs_species_code = 1011, predict_variable = "D_i", predict_category = 0, predict_vessel = 0, predict_covariates_df_all = pred_df, cov_names = c("Depth", "SST_seasonal", "BT_seasonal"), time_col = "Year", out_dir = here::here("", "results/prediction_df"))
```

This is certainly helpful, though leaves a bit to be desired as it would be great if we could make a prediction for different derived quantities -- like the startified biomass indices. 
