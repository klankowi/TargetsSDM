...
# Source a bunch of "vast" helper function. These are all located here: https://github.com/aallyn/TargetsSDM. Working on getting this into a nicer package style.
source_url("https://raw.githubusercontent.com/aallyn/TargetsSDM/main/R/nmfs_functions.R")
source_url("https://raw.githubusercontent.com/aallyn/TargetsSDM/main/R/dfo_functions.R")
source_url("https://raw.githubusercontent.com/aallyn/TargetsSDM/main/R/combo_functions.R")
source_url("https://raw.githubusercontent.com/aallyn/TargetsSDM/main/R/covariate_functions.R")
...
# Setting directories for the raw data files -- download and extract the zipped google drive folder to main_dir/data
nmfs_dat_dir<- paste0(main_dir, "/data/nmfs/raw")
dfo_dat_dir<- paste0(main_dir, "data/dfo/raw")
...
species<- read_csv(paste0(main_dir, "data/supporting/species_table.csv"))
...
static_dir<- paste0(main_dir, "data/covariates/static")
...
dynamic_dir<- paste0(main_dir, "data/covariates/dynamic")
...
region_shape<- st_read(paste0(main_dir, "data/supporting/region_shapefile/full_survey_region.shp"))
...
for(i in seq_along(index_areas)){
  index_area_temp<- st_read(paste0(main_dir, "data/supporting/index_shapefiles/", index_areas[i], ".shp"))
...
vast0<- vast_build_sdm(settings = vast_settings, extrap_grid = vast_extrap_grid_nona, sample_data = vast_sample_data, covariate_data = NULL, X1_formula = NULL, X2_formula = NULL, Q1_formula = NULL, Q2_formula = NULL, Xconfig_list = vast_coveff, X_contrasts = NULL, index_shapes = index_area_shapes, spatial_info_dir = main_dir)
```
...
land_use<- st_read(paste0(main_dir, "data/supporting/land_shapefile/ne_50m_land.shp"))
xlim_use<- c(-80, -55)
ylim_use<- c(35, 50)
```
...
```{r}
# Making the plots
spat_grid<- raster(paste0(main_dir, "data/supporting/Rast0.25grid.grd"))
design_plots<- vast_plot_design(vast_fit = vast_fitted, land = land_use, spat_grid = spat_grid, xlim = xlim_use, ylim = ylim_use, land_color = "#f0f0f0", out_dir = main_dir)
design_plots
```
...
# AIC
vast_fitted$parameter_estimates$AIC
```

Next, after checking the model fit to the data, we can look at parameter estimates. For the fixed effect parameters, we can get their MLE as well as standard errors. For this first model, this is relatively simple since we just have a single fixed effect parameter for each year time step in the data.
...
biomass_indices<- get_vast_index_timeseries(vast_fit = vast_fitted, nice_category_names = "Atlantic halibut", index_scale = c("raw"), out_dir = main_dir)

plot_vast_index_timeseries(index_res_df = biomass_indices, index_scale = "raw", nice_category_names = "Atlantic halibut", nice_xlab = "Year", nice_ylab= "Biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = main_dir)
```
...
```{r}
# Build
vast0_hab_covs<- vast_build_sdm(settings = vast_settings, extrap_grid = vast_extrap_grid_nona, sample_data = vast_sample_data, covariate_data = vast_covariate_data, X1_formula = hab_formula, X2_formula = hab_formula, Q1_formula = NULL, Q2_formula = NULL, Xconfig_list = vast_coveff_habcovs, X_contrasts = NULL, index_shapes = index_area_shapes, spatial_info_dir = main_dir)
```

After confirming everything looks right, we can fit the new model. 
```{r}
# Fit
vast_fitted_hab_covs<- vast_fit_sdm(vast_build_adjust = vast0_hab_covs, nmfs_species_code = 101, index_shapes = index_area_shapes, spatial_info_dir = main_dir, out_dir = main_dir)
```

### Model inference
As with our basic model, we can do a whole bunch of things after we've fit the model. For example, we could look at the estimated biomass and see how that compares to the basic model.
```{r}
biomass_indices_habcovs<- get_vast_index_timeseries(vast_fit = vast_fitted_hab_covs, nice_category_names = "Atlantic halibut", index_scale = c("raw"), out_dir = main_dir)
...
plot_vast_index_timeseries(index_res_df = biomass_indices_both, index_scale = "raw", nice_category_names = "Atlantic halibut", nice_xlab = "Year", nice_ylab= "Biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = main_dir)
...
```{r}
vast_habcovs_effs<- get_vast_covariate_effects(vast_fit = vast_fitted_hab_covs, params_plot = c("Depth", "SST_seasonal", "BT_seasonal"), params_plot_levels = 100, effects_pad_values = c(), nice_category_names = "Atlantic halibut", out_dir = main_dir)

# Warnings are...interesting....
vast_habcovs_plot<- plot_vast_covariate_effects(vast_covariate_effects = vast_habcovs_effs, vast_fit = vast_fitted_hab_covs, nice_category_names = "Atlantic halibut", out_dir = main_dir)
vast_habcovs_plot
```
